...
model_application <- function(persons, data1, coefs_name_mile, coefs_mile , coefs_names, coefs_vehs,
  ...
  mdcev_predicted <- VehicleFleet_model(data_temp, coef3_model, mdcev_model_spec_file)

  mdcev <- dcast(mdcev_predicted, ID ~ outcome, value.var = "cont_mean")
  mdcev[, AVG := 1]
