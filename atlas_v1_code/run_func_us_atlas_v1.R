#  us_atals_v1 run function by county

model_application <- function(select.state.fips, select.county.fips, data,
                             coefs_name_mile, coefs_mile , coefs_names, coefs_vehs,
  ...
  require(stats) # LJ add, predict function is used for predicting main driver and ownlease
  require(data.table)
  require(tidyverse)
  require(dplyr)
  require(apollo)
  ...
  while ((iter <= max_iteration) & (diff_body[1] > Tolerance | 
    ...
    pnumpercent <- tt2[[2]];
    pavgmileage <- tt2[[3]];
    tmp3 <- NumBodytypes(data_temp, indfor, pnumpercent, pavgmileage, coef_names_type, coef_values_type, diff_body);
    tt3 <- tmp3[[1]] # the dataframe
    diff_body <- tmp3[[2]] # the difference vector
  ...
  if(nrow(mod.pred) >0){
    ...
    iter.number = 1 # record how many times of iteration
    while(err.metric.power > power.threshold & iter.number <5){ # log adjust the constant until the predicted power matches the DMV control fractions
  ...
  fwrite(data_temp, file = file.path(outputdir, paste0("prediction_results_",select.state.fips,'_',select.county.fips,".csv")), row.names=FALSE, sep=",");
  fwrite(households_output, file = file.path(outputdir, paste0("atlas_hh_",select.state.fips,'_',select.county.fips,".csv")), row.names=FALSE, sep=",") 
  fwrite(vehicles_output, file = file.path(outputdir, paste0("atlas_veh_",select.state.fips,'_',select.county.fips,".csv")), row.names=FALSE, sep=",") 
  
  print(paste(select.state.fips, select.county.fips,"Successfully saved!"))
