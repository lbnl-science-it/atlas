...
if(useparser){
  ...
  option_list <- list(
    ...
    # LJ add 10/25/2025, mandatory option to specify state, as a two char code e.g. '06' for CA
    make_option(c("--state"), dest="state", action="store",
                help="the state fips code", default="06" ) ,
  ...
  print ( "ATLAS running mode for state fips")
  print( opt$state )
  ...
  select.state.fips = sprintf("%02d", as.integer(opt$state)) # e.g. '06' for CA
...
if(!useparser){ # if not using parser, define things here for debuging process
  ...
  outputyear <- 2017 # only place to change
  simyear = outputyear
...
if(atlas_runmod == 1) {
  ...
  source('source_us_atlas_v1.R') # change from source_atlas_v1.R to source_us_atlas_v1.R
