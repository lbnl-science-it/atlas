#============= step 6_0.5: predict main driver for new hh and newly acquired vehicles for middle year
maindriver0.5 <- function(vehicle, person){  
  require(data.table)
  require(stats)
...
p_maindriver0.5 <- function(vehicle, person, Npe = Npe){ # LJ 10/18/2022: parallel version
  require(data.table)
  require(stats)
  library(tidyverse)
  library(dplyr)
  ...
  res <- foreach(i=1:(length(tmpi)-1), 
    ...
    maindriver0.5(tmpveh, person=person)
  }
